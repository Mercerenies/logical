...
testAccessors = TestLabel "testAccessors" $ TestList [
    ...
    TestCase ([TermInt 999] @=? factBody (Fact "foo" [TermInt 999]))
  ]

testFree :: Test
testFree = TestLabel "testFree" $ TestList [
    TestCase (assertEqNoOrd "" [] (freeVars (TermInt 1))),
    ...
    TestCase (assertEqNoOrd "" ["A", "B", "C"]
                               (freeVars (TermCompound "foo" [TermVar "A", TermVar "B", TermInt 0, TermVar "C"]))),
    TestCase (assertEqNoOrd "" ["Foo", "Bar"]
                               (freeVars (TermCompound "foo" [TermCompound "bar" [TermVar "Bar", TermInt 0],
                                                              TermVar "Foo"])))
  ]
...
testFreeFact = TestLabel "testFreeFact" $ TestList [
    ...
    TestCase (assertEqNoOrd "" [] (freeVarsInFact (Fact "fact" [TermInt 9]))),
...
testRenameVars = TestLabel "testRenameVars" $ TestList [
    TestCase (TermInt 9 @=? renameVars [] (TermInt 9)),
    TestCase (TermCompound "foo" [] @=? renameVars [] (TermCompound "foo" [])),
    TestCase (TermInt 9 @=? renameVars ["A", "B"] (TermInt 9)),
